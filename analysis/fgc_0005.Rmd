...
desir_perf.depl <- readRDS(file="../data/desir_fdr_perf.rds")
mle_desir_0.607.depl <- readRDS(file="../data/mle_output_target_desir_0.607.rds")
...
fix_FDR.desir <- function(data, start_desir = 0.9, fdr_thresh = 0.1, type = "negative", step_size = 0.1){
  if(type == "negative"){
    data %<>%
  ...
  while(!finish){
    ...
    # Estimate of FDR after exluding singletons.
    ...
    FDR.comm <- (num_two_false + num_three_false)/num_TP
...
fdr.desir.sp <- fix_FDR.desir(data.desir, start_desir = 0.7, step_size = 0.02)
