...
fix_FDR.desir <- function(data, start_desir = 0.9, fdr_thresh = 0.05, type = "negative", step_size = 0.1){
  if(type == "negative"){
    data %<>%
  ...
  while(!finish){
    ...
    # Estimate of FDR.
    num_FP <- mle$performance$p10*nrow(td)
    num_TP <- mle$performance$p11*nrow(td)
    ...
    track_perf <- rbind(track_perf, data.frame(Desir_threshold = desir_thresh,
                                               ...
                                               Prob_pos.two_shared_hits = mle$delta$delta_pos[which(td$total==2)[1]],
...
fdr.desir <- fix_FPR.desir(data.desir)
fdr.desir.sp <- fix_FPR.desir(data.desir, start_desir = 0.7, step_size = 0.02)

gene_desi_0.607 <- prep_4_mle(data.desir,0.607) %>%
  mutate(probability_hit = vv$delta$delta_pos)
