...
# Ubiquitylome.
lscc.ubiq_pep <- read.delim("../data/CPTAC_S058_LSCC_Discovery/CPTAC3_Lung_Squamous_Cell_Carcinoma_Ubiquitylome.ubiquitylpeptide.tmt11.tsv",
                            stringsAsFactors = F)
...
```{r}
# Function library.
make_tumour_pair_diffs <- function(data, samps, add_extra_cols = T, ubiq = FALSE){
  colnames(data) <- gsub("^(.*?)\\..*$","\\1",colnames(data))
  samps <- samps[(samps$Tumour %in% colnames(data) & samps$NAT %in% colnames(data)),]
  if(!ubiq){
    tum <- unlist(data[,match(samps$Tumour, colnames(data))])
    nat <- unlist(data[,match(samps$NAT, colnames(data))])
  }else{
    tum <- apply(data[,match(samps$Tumour, colnames(data))],2,median,na.rm=T)
    nat <- apply(data[,match(samps$NAT, colnames(data))],2,median,na.rm=T)
  }
  if(add_extra_cols){
    ...
    ret <- data.frame(patient = 1:nrow(tum), tumour = tum,
...
mcorr_gene_set <- function(data, genes = NULL, n = 13){
  if(is.null(genes))
    genes <- sample(unique(data$Gene),n,replace = F)
...
lscc.all %>%
  ggplot(aes(synth_lethal,lfc_diff)) +
...
print(gppl +
        ggtitle("LSCC: Protein abundance correlations: synthetic lethals"))
...
```{r}
lscc.ubiq_pep.sl <- lscc.ubiq_pep %>%
  select(Gene,contains("Log.Ratio")) %>%
  filter(Gene %in% c("LIMD1","GPX1","GPX7","SLC7A11",sl.cand$gene[sl.cand$Gain_lethality_LIMD1==1])) %>%
  group_by(Gene) %>%
  do(data.frame(make_tumour_pair_diffs(., lscc.samps, ubiq=T))) %>%
  ungroup()

# Summarise multiple peptides for each gene.
lscc.ubiq_pep.sl.summ <- lscc.ubiq_pep.sl %>%
  group_by(patient, Gene) %>%
  summarise(log_ratio_diff.median = median(lfc_diff,na.rm=T)) %>%
  ungroup()

lscc.ubiq_pep.sl.diff <- lscc.ubiq_pep.sl.summ %>%
  pivot_wider(names_from = Gene, values_from = log_ratio_diff.median)

lscc.ubiq_pep.sl.diff %>%
  ggplot(aes(LIMD1,GPX4)) +
  geom_point() +
  geom_smooth()
...
# Plots.
cpl <- ggcorr(luad.sl.diff %>% 
                select(-patient,-EEFSEC,-GPX7,-SNAP23), 
              label=T)
cpl.g <- ggplotGrob(cpl)
cpl.colors <- cpl.g$grobs[[6]]$children[[3]]$gp$fill

gppl <- ggpairs(luad.sl.diff %>%
                   select(-patient,-EEFSEC,-GPX7,-SNAP23), 
                columns = 1:12, lower = list(continuous = smf))
...
print(gppl +
        ggtitle("LUAD: Protein abundance correlations: synthetic lethals"))
