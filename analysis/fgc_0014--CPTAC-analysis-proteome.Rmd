...
# CPTAC samples: organised into tumour-non-tumour pairs.
...
```{r}
# Function library.
make_tumour_pair_diffs <- function(data){
  colnames(data) <- gsub("^(.*?)\\..*$","\\1",colnames(data))
  tum <- unlist(data[,colnames(data) %in% lscc.samps$Tumour])
  nat <- unlist(data[,colnames(data) %in% lscc.samps$NAT])
...
lscc.sl <- lscc %>%
  ...
  group_by(Gene) %>%
  do(data.frame(make_tumour_pair_diffs(.))) %>%
  ungroup() %>%
