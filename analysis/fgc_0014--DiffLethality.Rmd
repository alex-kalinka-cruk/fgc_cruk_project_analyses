...
plot_DiffLeth <- function(data, screen_name){
  # Remove genes with multimapping guides.
  data$par_limd1_essential %<>%
    dplyr::filter(!id %in% crispr_libs_annot_data$knockout$yusa_v3_human$v1$multimapping_guides$genes &
                    id != "Control")
  ...
  if("Gain-Ess_LIMD1" %in% data$Diff_lethality$typeU){
    paired <- paired +
...
```{r}
gain_hits_union <- unique(c((day10_scr1$Diff_lethality %>% dplyr::filter(typeU == "Gain-Ess_LIMD1"))$id,
                     (day17_scr1$Diff_lethality %>% dplyr::filter(typeU == "Gain-Ess_LIMD1"))$id,
                     (day11_scr2$Diff_lethality %>% dplyr::filter(typeU == "Gain-Ess_LIMD1"))$id,
                     (day21_scr2$Diff_lethality %>% dplyr::filter(typeU == "Gain-Ess_LIMD1"))$id))
```
...
```{r}
depleted_ess_hits_union <- unique(c((day10_scr1$Diff_lethality %>% 
                                   dplyr::filter(neg.fdr < 0.1 & 
                                                   id %in% day10_scr1$LIMD1_essential_union))$id,
                     (day17_scr1$Diff_lethality %>% 
                        dplyr::filter(neg.fdr < 0.1 &
                                        id %in% day17_scr1$LIMD1_essential_union))$id,
                     (day11_scr2$Diff_lethality %>% 
                        dplyr::filter(neg.fdr < 0.1 & 
                                        id %in% day11_scr2$LIMD1_essential_union))$id,
                     (day21_scr2$Diff_lethality %>% 
                        dplyr::filter(neg.fdr < 0.1 & 
                                        id %in% day21_scr2$LIMD1_essential_union))$id))
...
```{r}
depleted_hits_union <- unique(c((day10_scr1$Diff_lethality %>% 
                                   dplyr::filter(neg.fdr < 0.1))$id,
                     (day17_scr1$Diff_lethality %>% 
                        dplyr::filter(neg.fdr < 0.1))$id,
                     (day11_scr2$Diff_lethality %>% 
...
```{r}
loss_hits_union <- unique(c((day10_scr1$Diff_lethality %>% dplyr::filter(typeU == "Loss-Ess_LIMD1"))$id,
                     (day17_scr1$Diff_lethality %>% dplyr::filter(typeU == "Loss-Ess_LIMD1"))$id,
                     (day11_scr2$Diff_lethality %>% dplyr::filter(typeU == "Loss-Ess_LIMD1"))$id,
                     (day21_scr2$Diff_lethality %>% dplyr::filter(typeU == "Loss-Ess_LIMD1"))$id))
...
plot_perf_curves_fgcQC(qc, "PrRc")
