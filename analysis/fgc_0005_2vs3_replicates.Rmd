...
Using Efron's local FDR approach (Efron 2007) and the accompanying R package `locfdr` to estimate the power of 3 vs 2 replicates at an FDR of 10%. 

```{r}
lf_3reps <- locfdr(mh$neg.lfc[mh$cell_line=="HCT116"], df=15, plot=3)
lf_2reps.1 <- locfdr(mh$neg.lfc[mh$cell_line=="HCT116_2replicates_combn1"], df=15, plot=3)
lf_2reps.2 <- locfdr(mh$neg.lfc[mh$cell_line=="HCT116_2replicates_combn2"], df=15, plot=3)
lf_2reps.3 <- locfdr(mh$neg.lfc[mh$cell_line=="HCT116_2replicates_combn3"], df=15, plot=3)
...
roc %>%
  ...
  ggtitle("HCT116: replicate power comparison")
