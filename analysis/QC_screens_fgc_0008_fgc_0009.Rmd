...
lfc_cleanr <- read_cleanr_lfc(cleanr_lfc.9.files, "FGC_0009") %>%
  mutate(time_point = factor(time_point, levels = c("baseline","d7","d17")))
...
lfc_cleanr %>%
  ...
  ggplot(aes(gene_group, correctedFC, color = gene_group)) +
    geom_point() +
