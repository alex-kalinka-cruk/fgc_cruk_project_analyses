...
author: 
- "Alex Kalinka"
- "FGC, Milner Therapeutics Institute, Cambridge, UK"
...
* HT-29 is an outlier among the 4 colon cancer cell lines - this is corroborated by a recent paper classifying HT-29 in a different group to the other 3 cell lines.
...
![The probability that a gene is a true hit is enhanced by occurring in more than 1 cell line screen. $P_{11}$ - probability that a hit is a true positive.](../images/Celllines_Alex.png)
...
```{r}
# Save combined hits with extra column indicating shared/singleton.
write.csv(combined_desir %>%
            filter(hit) %>%
            mutate(hit_type = ifelse(Gene %in% cand_df$Gene,"Shared","Singleton")) %>%
            arrange(desc(Desirability)) %>%
            rename(Desirability_median = Desirability,
                   Desirability_HCT116 = D_HCT116,
                   Desirability_SW480 = D_SW480,
                   Desirability_RKO = D_RKO,
                   `Desirability_HT-29` = `D_HT-29`),
          file = "../data/fgc_0005_depletion_hit_table.csv", 
          row.names = F, quote = F)
